...
if ($_SESSION["TL"]<>0)
{
	...
	{
		header('Location: chem_mocks.php');
	}
}
else
{
...
$sql ="select sr_no from mocks_q_chem where status='A' and type like '%".$_SESSION["type"]."%' and sr_no in (select sr_no from mocks_asso_mocks_q_test_chem where test_no=".$_GET["tno"].")";
#echo $sql."<BR>";
...
if (!isset($_GET["qno"]))
{
	#get attempt number	
	$_SESSION["attempt"]=1;
	$sql_ ="select max(attempt) as ma from mocks_users_ans_chem where uname='".$_SESSION["uname"]."' and test_no=".$_GET["tno"];
...
else
{
	...
	if(!isset($_GET["ans"]))
	{
			$sql_a="select op_no from mocks_users_ans_chem where uname='".$_SESSION["uname"]."' and test_no=".$_GET["tno"]." and attempt=".$_SESSION["attempt"]." and q_sr_no=".$cqno;
		...
		$cans=$_GET["ans"];
	#sql update here
	$sql="update mocks_users_ans_chem set op_no=".$cans.", ans_date_time=sysdate() where uname='".$_SESSION["uname"]."' and test_no=".$_GET["tno"]." and attempt=".$_SESSION["attempt"]." and q_sr_no=".$cqno;
...
{
	$qno=$_GET["qno"];
	$sql ="select * from mocks_q_chem where sr_no=".$qno;
	$result = mysqli_query($conn, $sql);
	if (mysqli_num_rows($result) > 0) 
	{
		...
		{
			$sql_a="select op_no from mocks_users_ans_chem where uname='".$_SESSION["uname"]."' and test_no=".$_GET["tno"]." and attempt=".$_SESSION["attempt"]." and q_sr_no=".$qno;
			#echo $sql_a."<BR>";
			$result22= mysqli_query($conn, $sql_a);
			$prev_ans=0;
			if($row22 = mysqli_fetch_assoc($result22))
			...
			$sql ="select * from mocks_o_chem where sr_no=".$selected_q_no." order by op_no";
			$result2= mysqli_query($conn, $sql);
			echo "<form action=give_chem_test_BCK.php method=get><input type=hidden name=tno value=".$_GET["tno"]."><input type=hidden name=qno value=".$qno.">";
			$op_cnt=0;
			...
			echo "<HR>";		
			$sql="select max(id) as c from mocks_users_ans_chem";
			...
			$sql="delete from mocks_users_ans_chem where uname='".$_SESSION["uname"]."' and test_no=".$_GET["tno"]." and attempt=".$_SESSION["attempt"]." and (q_sr_no=".$qno." or ans_date_time=0000-00-00)";
			#echo $sql."<BR>";
			if (mysqli_query($conn, $sql))
			{
			...
			if ($conn->connect_error)
				die("Connection failed: " . $conn->connect_error);
			$sql="insert into mocks_users_ans_chem (id,uname, test_no, attempt,q_sr_no, start_date_time) values (".$id.",'".$_SESSION["uname"]."',".$_GET["tno"].",".$_SESSION["attempt"].",".$qno.",sysdate())";
			#echo "ISNERTING....".$sql;
			if (mysqli_query($conn, $sql))
			{
...
if(isset($_SESSION["QL"]))
{
	...
	{
		if ($i%10==0) echo "<BR>";
		echo "<a href='give_chem_test_BCK.php?tno=".$_GET["tno"]."&qno=$quest[$i]&showquest=1'>".$i."</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
	}
}
