...
else
{
	...
	if(!isset($_GET["ans"]))
	{
			$sql_a="select op_no from mocks_users_ans where uname='".$_SESSION["uname"]."' and test_no=".$_GET["tno"]." and attempt=".$_SESSION["attempt"]." and q_sr_no=".$cqno;
...
if (isset($_GET["showquest"]))
{
	...
	if (mysqli_num_rows($result) > 0) 
	{
		...
		{
			$sql_a="select op_no from mocks_users_ans where uname='".$_SESSION["uname"]."' and test_no=".$_GET["tno"]." and attempt=".$_SESSION["attempt"]." and q_sr_no=".$qno;
			#echo $sql_a."<BR>";
			$result22= mysqli_query($conn, $sql_a);
			$prev_ans=0;
			if($row22 = mysqli_fetch_assoc($result22))
			...
			echo "<form action=give_physics_test_BCK.php method=get><input type=hidden name=tno value=".$_GET["tno"]."><input type=hidden name=qno value=".$qno.">";
			$op_cnt=0;
			...
			echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=submit value='Submit Answer'><font color=red><B><==PRESS SUBMIT to submit above answer</B></font></form>";
			echo "<HR>";		
			...
			$sql="delete from mocks_users_ans where uname='".$_SESSION["uname"]."' and test_no=".$_GET["tno"]." and attempt=".$_SESSION["attempt"]." and (q_sr_no=".$qno." or ans_date_time=0000-00-00)";
			#echo $sql."<BR>";
			if (mysqli_query($conn, $sql))
			{
...
if(isset($_SESSION["QL"]))
{
	...
	{
		if ($i%10==0) echo "<BR>";
		echo "<a href='give_physics_test_BCK.php?tno=".$_GET["tno"]."&qno=$quest[$i]&showquest=1'>".$i."</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
	}
}
