...
$sql ="select * from mocks_asso_users_test_maths, mocks_test_maths where mocks_asso_users_test_maths.test_no=mocks_test_maths.test_no and uname='".$_SESSION["uname"]."' and status='A' order by mocks_test_maths.test_no";
...
if (mysqli_num_rows($result) > 0) 
{
	...
	while($row = mysqli_fetch_assoc($result))
	{
		...
		if($status==1)
			if (isset($_SESSION["BCK"])) echo "<td><h1><a href=give_maths_test_BCK.php?tno=".$row["test_no"]."&ql=".$_SESSION["QL"].">Click HERE to START</a></h1>";
			else echo "<td><h1><a href=give_maths_test.php?tno=".$row["test_no"]."&ql=".$_SESSION["QL"].">Click HERE to START</a></h1>";
	//	echo "<td><h1><a href=give_maths_test.php?tno=".$row["test_no"]."&ql=".$_SESSION["QL"].">Click HERE to START</a></h1>";
		else if($status==0)
		echo "<td><B>Test OVER</b>";

		#displaying scores
		$sql2 ="select mocks_users_ans_maths.op_no as ua, mocks_a_maths.op_no as a, attempt, mocks_a_maths.sr_no as q from mocks_users_ans_maths, mocks_a_maths where mocks_users_ans_maths.q_sr_no=mocks_a_maths.sr_no and uname='".$_SESSION["uname"]."' and test_no=".$row["test_no"]." order by mocks_users_ans_maths.attempt,mocks_users_ans_maths.q_sr_no";
		#echo "<BR>".$sql2."<BR>";
		...
		if (mysqli_num_rows($result2) > 0) 
		{
			...
			while($row2 = mysqli_fetch_assoc($result2))
			{
				if($curr_attempt!=-1 && $curr_attempt!=$row2["attempt"])
				{
					...
					if($status==0)
						echo " <a href='the_analyst_maths.php?test_no=".$row["test_no"]."' target=_black>Analysis</a>";
